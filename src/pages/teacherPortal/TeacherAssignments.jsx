...
const TeacherAssignments = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError("");
      if (editingId) {
        await updateTeacherAssignment(editingId, form);
      } else {
      ...
      <div className="space-y-6">
        <form className={`admin-card p-5 md:p-6 ${editingId ? "admin-edit-mode" : ""}`} onSubmit={submit}>
          <div className="mb-5">
            ...
            {/* <p className="text-sm text-slate-600 mt-1">Create professional assignments with title, description, and deadline date.</p> */}
          </div>
          <input className="admin-input mb-3" placeholder="Assignment title" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} required />
          <textarea className="admin-input mb-3 min-h-28" placeholder="Assignment description" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} required />
        ...
        <div className="admin-card p-0 overflow-hidden">
          <div className="px-5 py-4 border-b border-emerald-100 bg-gradient-to-r from-emerald-50 to-white">
            ...
            <p className="text-sm text-slate-500 mt-1">
              All active assignments published for your assigned class.
            </p>
          </div>
          <div className="grid gap-3 p-4 md:hidden">
            ...
            {!loading && assignments.length === 0 ? (
              <div className="rounded-xl border border-gray-100 bg-white p-4 text-sm text-slate-500">No assignments found.</div>
            ) : null}
            {!loading
              ? assignments.map((a) => (
                  <article key={a.Id} className="rounded-xl border border-emerald-100 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3 border-t border-gray-100 pt-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button type="button" className="admin-action-icon-btn is-edit" onClick={() => startEdit(a)} aria-label="Edit assignment" title="Edit">
                          <img src="/edit.svg" alt="" className="admin-action-icon" />
                        </button>
                        <button type="button" className="admin-action-icon-btn is-delete" onClick={() => handleDelete(a)} disabled={deletingId === a.Id} aria-label="Delete assignment" title={deletingId === a.Id ? "Deleting" : "Delete"}>
